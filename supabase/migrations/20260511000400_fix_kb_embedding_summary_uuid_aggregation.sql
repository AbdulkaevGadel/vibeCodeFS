-- Fix UUID aggregation in KB embedding summary/start batch RPCs.
-- 20260511000300 was already applied, so this is an incremental replacement.
    ...
    with article_state as (
        select
            ...
            (array_agg(current_set.id order by current_set.created_at desc, current_set.id desc)
                filter (where current_set.id is not null))[1] as current_id,
            (array_agg(current_set.status order by current_set.created_at desc, current_set.id desc)
                filter (where current_set.id is not null))[1] as current_status,
