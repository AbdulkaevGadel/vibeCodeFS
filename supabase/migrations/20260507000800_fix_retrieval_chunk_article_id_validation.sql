-- Runtime fix: accept valid UUID article_id values in retrieval chunks.
--
-- Purpose:
-- - RAG hit runs can be rejected as invalid_retrieval_chunks during persistence.
-- - The persistence RPC is the authoritative validator for retrieval_chunks.
-- - Keep the RPC contract and retrieval policy unchanged, but ensure article_id
--   uses the same full UUID validation as chunk_id: 8-4-4-4-12.
...
alter function public.save_chat_ai_retrieval_result(uuid, text, text, double precision, integer, jsonb, text, text)
owner to postgres;

revoke all on function public.save_chat_ai_retrieval_result(uuid, text, text, double precision, integer, jsonb, text, text)
from public, anon, authenticated;

grant execute on function public.save_chat_ai_retrieval_result(uuid, text, text, double precision, integer, jsonb, text, text)
to service_role;

comment on function public.save_chat_ai_retrieval_result(uuid, text, text, double precision, integer, jsonb, text, text) is
    'Only approved write path for chat_ai_runs retrieval fields. Validates backend-only retrieval diagnostics with full UUID chunk and article ids.';
