-- Runtime blocker fix: DB-side timeout guard for hybrid retrieval RPC.
--
-- Purpose:
-- - Direct DB calls to match_knowledge_chunks_v1 return quickly.
-- - Edge -> PostgREST RPC calls can still hang at retrieval_rpc_started until EarlyDrop.
-- - The retrieval RPC must have its own DB-side execution guard.
--
-- This migration keeps the existing retrieval policy, ranking, diagnostic fields,
-- owner, SECURITY DEFINER behavior, and service_role-only execute boundary.
...
comment on function public.match_knowledge_chunks_v1(vector, text, double precision, integer, integer) is
    'Policy-enforced Knowledge Base hybrid retrieval with backend-only diagnostic fields and DB-side statement timeout guard.';
