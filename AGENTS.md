...
Перед предложением или написанием нового кода агент должен изучить текущую структуру затронутой части проекта и следовать существующему стилю организации, если нет явной причины менять его.

Rules:
- не вводить новый паттерн папок, если в затронутой зоне уже есть понятная локальная структура;
- предпочитать расширение существующей route, feature, function или domain-структуры вместо создания параллельной структуры;
- если текущая структура является переходной, следовать утверждённому целевому направлению из локального `AGENTS.md`;
- если изменение структуры действительно нужно, сначала объяснить, почему текущая структура недостаточна, и получить одобрение до перемещения файлов или введения нового паттерна.
