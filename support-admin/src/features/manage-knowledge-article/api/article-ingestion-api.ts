...
export function getKnowledgeArticleActionErrorMessage(error: unknown, fallback: string) {
  return error instanceof Error && error.message ? error.message : fallback;
}

export async function invokePendingArticleIngestionIfNeeded(articleId: string | null) {
  if (!articleId) {
    return;
  }
